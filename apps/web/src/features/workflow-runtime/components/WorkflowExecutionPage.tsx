...
const parseWorkflowExecutionStatusParam = (value: string | null): WorkflowExecutionStatus | undefined => {
    if (value === 'PENDING' || value === 'RUNNING' || value === 'SUCCESS' || value === 'FAILED' || value === 'CANCELED') {
        return value;
    }
    return undefined;
};
...
const parseWorkflowTriggerTypeParam = (value: string | null): WorkflowTriggerType | undefined => {
    if (value === 'MANUAL' || value === 'API' || value === 'SCHEDULE' || value === 'EVENT' || value === 'ON_DEMAND') {
        return value;
    }
    return undefined;
};
...
const parseWorkflowRiskDegradeActionParam = (value: string | null): WorkflowRiskDegradeAction | undefined => {
...
const parseRiskGateSummaryRecord = (raw: Record<string, unknown> | null): RiskGateSummary | null => {
    ...
    return {
        ...
        blockedByRiskLevel: readBoolean(raw.blockedByRiskLevel) ?? readBoolean(riskGateMeta?.blockedByRiskLevel),
...
export const WorkflowExecutionPage: React.FC = () => {
    ...
    const [selectedDegradeAction, setSelectedDegradeAction] = useState<WorkflowRiskDegradeAction | undefined>(() =>
    ...
    const [selectedRiskSummaryPresence, setSelectedRiskSummaryPresence] = useState<boolean | undefined>(() =>
                                ...
                                <Tooltip
                                    title={[
                                        ...
                                        summary.blockers.length
                                            ? `阻断项: ${summary.blockers.join(', ')}`
                                            : null,
                                        summary.degradeAction
                                            ? `降级动作: ${summary.degradeAction}`
                                            : null,
                                        summary.threshold
                                            ? `阈值: ${summary.threshold}`
                                            : null,
                                        summary.riskProfileCode
                                            ? `风控模板: ${summary.riskProfileCode}`
                                            : null,
                            ...
                            },
                            {
                                ...
                                children: riskGateSummary ? (
                                    <Tag color={riskGateSummary.passed ? 'success' : 'error'}>
                                        {riskGateSummary.passed
                                            ? '通过'
                                            : riskGateSummary.blocked
                                                ? '阻断'
                                                : '待定'}
                                    </Tag>
                                ) : '-',
