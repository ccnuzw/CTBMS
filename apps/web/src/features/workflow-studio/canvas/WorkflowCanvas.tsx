...
const runtimePresetPolicyMap: Record<
  RuntimePreset,
  {
    timeoutMs: number;
    retryCount: number;
...
const WorkflowCanvasInner: React.FC<WorkflowCanvasProps> = ({
  ...
  const [menuState, setMenuState] = useState<{ id: string; top: number; left: number } | null>(
    null,
  );
  ...
  const currentDslSnapshot = useMemo(() => exportDsl(), [exportDsl]);
          ...
          <div>
            {stepReports.map((item, index) => (
              <div key={`${item.title}-${index}`}>
                {index + 1}. {item.title}，执行 {item.actions.length} 项，剩余{' '}
                {item.remainingIssueCount} 项
              </div>
            ))}
          </div>
        ),
  ...
  const handleApplyRuntimePreset = useCallback(
    (preset: RuntimePreset) => {
      ...
      message.success(
        `已批量应用${preset === 'FAST' ? '快速' : preset === 'ROBUST' ? '稳健' : '平衡'}运行策略`,
      );
    },
    ...
    <div style={{ display: 'flex', width: '100%', height: '100%' }}>
      ...
      >
        {!isReadOnly && (
          <CanvasToolbar
            ...
            onOpenTelemetrySummary={() => setTelemetryDrawerOpen(true)}
          />
        )}
        ...
        <WorkflowPreviewTelemetryDrawer
          ...
          onFocusNode={(nodeId) => {
            ...
            if (!node) {
              message.warning(`节点 ${nodeId} 不在当前画布（可能属于其他流程版本）`);
              return;
            }
            setSelectedNodeId(nodeId);
            setSelectedEdgeId(null);
      ...
      {!isReadOnly && (selectedNode || selectedEdge) ? (
        <PropertyPanel
          ...
          viewLevel={currentViewLevel}
          paramSetBindings={initialDsl?.paramSetBindings ?? []}
          currentDsl={currentDslSnapshot}
          onFocusNode={(nodeId) => {
            const node = nodes.find((item) => item.id === nodeId);
            if (!node) {
              message.warning(`节点 ${nodeId} 不在当前画布（可能属于其他流程版本）`);
          ...
          {selectedNode && (
            <div>
              <p style={{ marginBottom: 8 }}>
                <Tag color="blue">{String(selectedNode.data.type)}</Tag>{' '}
                <strong>{String(selectedNode.data.name)}</strong>
              </p>
              <div style={{ marginBottom: 16 }}>
                <Alert
                  message={executionStatusMap.get(selectedNode.id) || 'PENDING'}
                  type={
                    executionStatusMap.get(selectedNode.id) === 'SUCCESS'
                      ? 'success'
                      : executionStatusMap.get(selectedNode.id) === 'FAILED'
                        ? 'error'
                        : 'info'
                  }
                  showIcon
