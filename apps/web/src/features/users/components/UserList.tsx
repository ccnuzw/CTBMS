...
export const UserList: React.FC = () => {
  ...
  const genderMap = useMemo(() => {
    ...
    return items.reduce<Record<string, { text: string; icon: React.ReactNode; color: string }>>(
      (acc, item) => {
        const icon =
          item.code === 'MALE' ? (
            <ManOutlined />
  ...
  const [tableData, setTableData] = useState<UserWithRelations[]>([]);
    ...
    >
      <Flex justify="space-between" align="flex-start">
        ...
        <Space size="small">
          ...
          <Popconfirm title="确定删除该用户吗?" onConfirm={() => handleDelete(user.id)}>
  ...
  const columns: ProColumns<UserWithRelations>[] = [
    ...
    },
    {
      ...
      render: (_, record) => [
        ...
        <Button key="roles" size="small" onClick={() => handleAssignRoles(record)}>
          角色
        </Button>,
  ...
  const requestUsers = async (params: {
    ...
    const usernameKeyword = params.username?.trim().toLowerCase();
    if (usernameKeyword) {
      data = data.filter((item) => item.username?.toLowerCase().includes(usernameKeyword));
    }

    const nameKeyword = params.name?.trim().toLowerCase();
    if (nameKeyword) {
      data = data.filter((item) => item.name?.toLowerCase().includes(nameKeyword));
    }

    const phoneKeyword = params.phone?.trim().toLowerCase();
    if (phoneKeyword) {
      data = data.filter((item) => item.phone?.toLowerCase().includes(phoneKeyword));
    }

    const emailKeyword = params.email?.trim().toLowerCase();
    if (emailKeyword) {
      data = data.filter((item) => item.email?.toLowerCase().includes(emailKeyword));
    }

    const statusFilters = Array.isArray(params.status)
      ? params.status
      : params.status
        ? [params.status]
        : [];
    if (statusFilters.length > 0) {
      ...
      data = data.filter((item) => item.status && statusSet.has(item.status));
    }
  ...
  return (
    <>
      <ProTable<UserWithRelations>
        ...
        loading={isLoading}
        columns={columns}
        onDataSourceChange={(next) => setTableData(next || [])}
        pagination={{
          defaultPageSize: 10,
          showSizeChanger: true,
        }}
        tableRender={(_, dom) => {
          if (!screens.md && tableData.length > 0) {
            return (
              <div style={{ padding: token.paddingSM }}>{tableData.map(renderMobileCard)}</div>
            );
          }
          return dom;
        }}
