  ...
  const chartData = useMemo(() => {
    ...
    multiPointData.forEach((item) => {
      item.data.forEach((point) => {
        ...
        const row = ensureRow(dateStr);
        row[item.point.name] = point.price;
        row[`${item.point.name}_change`] = point.change;
      });
    });

    if (regionData?.trend) {
      regionData.trend.forEach((regionPoint) => {
        const row = ensureRow(regionPoint.date);
        row[regionKey] = Math.round(regionPoint.avgPrice);
      });
    }
            ...
            >
              ...
              {latest && (
                <Text type="secondary">
                  ...
                  {latest.change !== null && ` (${latest.change > 0 ? '+' : ''}${latest.change})`}
                </Text>
              )}
            </Flex>
          );
    ...
    >
      ...
      {quality && (
        <Flex align="center" gap={8} style={{ marginBottom: 12 }}>
          ...
          {quality.latestDate && (
            <Tag color="default">最后更新 {dayjs(quality.latestDate).format('YYYY-MM-DD')}</Tag>
          )}
      ...
      {isError ? (
        <Flex justify="center" align="center" vertical style={{ height: 350, gap: 16 }}>
          <Empty
            ...
            description={
              <Text type="danger">数据加载失败: {(error as Error)?.message || '未知错误'}</Text>
            }
          />
