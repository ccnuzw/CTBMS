...
export const DataGrid: React.FC<DataGridProps> = ({
  ...
  useEffect(() => {
    ...
    (pointTypeFilter || []).join(','),
    (subTypes || []).join(','),
    (selectedQualityTags || []).join(','),
    reviewScope,
    sourceScope,
  ]);
  ...
  const filteredData = useMemo(() => priceDataList, [priceDataList]);
  ...
  const columns: ColumnsType<PriceDataRow> = [
    ...
    },
    {
      ...
      render: (subType: string) => (
        <Tag color="blue">{priceSubTypeLabels[normalizePriceSubTypeCode(subType)] || subType}</Tag>
      ),
  ...
  return (
    <div>
      <Flex justify="space-between" align="center" style={{ marginBottom: 12 }}>
        <Space size={8}>
          <Text type="secondary">共 {priceDataResult?.total || 0} 条记录</Text>
          {quality && (
            <Space size={6}>
      ...
      <Table
        ...
        onChange={(pagination) => {
          ...
          const nextPageSize = pagination.pageSize || 15;
