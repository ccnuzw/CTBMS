...
import { IntelTaskStatus, IntelTaskResponse, IntelTaskPriority } from '@packages/types';
...
export const MyTaskBoard: React.FC = () => {
  ...
  const taskPriorityMeta = useMemo(() => {
    ...
    if (!items.length)
      return { labels: {} as Record<string, string>, colors: {} as Record<string, string> };
  ...
  const taskStatusMeta = useMemo(() => {
      ...
      return {
        ...
        valueEnum: {
          [IntelTaskStatus.PENDING]: {
            text: fallbackLabels[IntelTaskStatus.PENDING],
            status: 'Processing',
          },
          [IntelTaskStatus.OVERDUE]: {
            text: fallbackLabels[IntelTaskStatus.OVERDUE],
            status: 'Error',
          },
          [IntelTaskStatus.COMPLETED]: {
            text: fallbackLabels[IntelTaskStatus.COMPLETED],
            status: 'Success',
          },
        },
      };
    ...
    const valueEnum = items.reduce<Record<string, { text: string; status: string }>>(
      (acc, item) => {
        const color = (item.meta as { color?: string } | null)?.color || 'default';
  ...
  const columns = [
    {
      title: taskStatusMeta.labels[IntelTaskStatus.PENDING] || '待办任务',
      status: IntelTaskStatus.PENDING,
      color: 'blue',
      icon: <ClockCircleOutlined />,
    },
    {
      title: taskStatusMeta.labels[IntelTaskStatus.OVERDUE] || '即将超时',
      status: IntelTaskStatus.OVERDUE,
      color: 'red',
      icon: <ExclamationCircleOutlined />,
    },
    {
      title: taskStatusMeta.labels[IntelTaskStatus.COMPLETED] || '已完成',
        ...
        <ProCard ghost gutter={16} direction="row" style={{ minWidth: 1000 }}>
          {columns.map((col) => (
            <ProCard
              ...
              extra={
                <Badge
                  count={groupedTasks[col.status]?.length}
                  style={{ backgroundColor: col.color }}
                />
              }
              headStyle={{ borderBottom: `2px solid ${col.color}` }}
              style={{ height: '100%', backgroundColor: token.colorFillQuaternary }}
                    ...
                    >
                      <Tooltip title={`完整标题: ${task.title}`}>
                        <Text strong ellipsis style={{ maxWidth: 180 }}>
                          ...
                          {(task as any).template?.name ||
                            task.title
                              .replace(/\[(港口|站台|企业|地域|批发市场)\].*?\[/g, '[')
                              .replace(/\[.*?\]\s*$/, '')}
                        </Text>
                      </Tooltip>
                      <Tag
                        color={
                          taskPriorityMeta.colors[task.priority] ||
                          (task.priority === IntelTaskPriority.URGENT ? 'red' : 'blue')
                        }
                      >
            ...
            },
            {
              ...
              render: (_, record) => [
                ...
                <Popconfirm
                  ...
                  description="取消后任务将从列表中移除"
                  onConfirm={() =>
                    cancelMutation.mutate({ id: record.id, operatorId: currentUser?.id })
                  }
                  okText="确定"
                  cancelText="取消"
                >
