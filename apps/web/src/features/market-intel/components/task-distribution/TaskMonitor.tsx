  ...
  const columns: ProColumns<IntelTaskResponse>[] = [
    ...
    },
    {
      ...
      render: (_, record) => {
        const colorMap = {
          ...
          [IntelTaskStatus.CANCELLED]: 'default',
        };
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        return (
          <Tag color={colorMap[record.priority]}>{INTEL_TASK_PRIORITY_LABELS[record.priority]}</Tag>
        );
      },
    },
    {
      ...
      render: (_, record) => {
        const isOverdue =
          dayjs().isAfter(dayjs(record.deadline)) && record.status !== IntelTaskStatus.COMPLETED;
        return (
