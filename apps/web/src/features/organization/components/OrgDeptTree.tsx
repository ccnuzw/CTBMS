...
import { useOrganizationTree, useCreateOrganization } from '../api/organizations';
import { useDepartments, useCreateDepartment } from '../api/departments';
...
export const OrgDeptTree: React.FC<OrgDeptTreeProps> = ({
  ...
  }, [orgTree, deptTreeMetaByOrg, token]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {onShowAllLevelsChange && (
          <Flex justify="space-between" align="center">
            <span style={{ fontSize: 12, color: token.colorTextSecondary }}>包含下级员工</span>
            <Switch size="small" checked={!!showAllLevels} onChange={onShowAllLevelsChange} />
          </Flex>
        )}
      </Flex>
      ...
      <ModalForm<CreateOrganizationDto>
        title={
          selectedNode?.type === 'org' ? `新增子组织 - ${selectedNode.name}下` : '新增公司/组织'
        }
        open={orgModalOpen}
