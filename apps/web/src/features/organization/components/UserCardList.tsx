...
import { useUsers, UserWithRelations, useBatchAssignUsers } from '../../users/api/users';
...
const getDescendantIds = (rootId: string, childrenMap: Map<string, string[]>) => {
  const result: string[] = [];
...
export const UserCardList: React.FC<UserCardListProps> = ({
  ...
  const batchAssignUsersMutation = useBatchAssignUsers();

  // 组织子节点索引（parentId -> childIds）
  const orgChildrenMap = useMemo(() => {
  ...
  // 部门子节点索引（parentId -> childIds）
  const deptChildrenMap = useMemo(() => {
    const map = new Map<string, string[]>();
    if (!allDepartments) return map;
  ...
  const userFilters = useMemo(() => {
    ...
    if (selectedNode.type === 'org') {
      if (showAllLevels) {
        const descendantOrgIds = getDescendantIds(selectedNode.id, orgChildrenMap);
        return {
          organizationIds: [selectedNode.id, ...descendantOrgIds],
          keyword,
        };
      }
    ...
    if (showAllLevels) {
      const descendantDeptIds = getDescendantIds(selectedNode.id, deptChildrenMap);
      return {
        departmentIds: [selectedNode.id, ...descendantDeptIds],
        keyword,
      };
    }
  ...
  }, [selectedNode, showAllLevels, orgChildrenMap, deptChildrenMap, keyword]);

  // 正常浏览只拉取范围内用户，避免全量用户下发到前端
  const {
    data: scopedUsers,
    isLoading,
    isFetching,
  ...
  const { data: usersForAssign } = useUsers(
    { unassigned: true },
    {
      enabled: !!selectedNode && assignModalOpen,
    },
  );

  // 获取未分配的用户
  const unassignedUsers = useMemo(() => {
    if (!usersForAssign) return [];
    return usersForAssign;
  }, [usersForAssign]);

  const filteredUsers = useMemo(() => scopedUsers || [], [scopedUsers]);
    ...
    try {
      const payload =
        selectedNode.type === 'org'
          ...
          : {
              userIds: selectedUnassignedUsers,
              organizationId: selectedNode.orgId,
              departmentId: selectedNode.id,
            };

      await batchAssignUsersMutation.mutateAsync(payload);
  ...
  return (
    ...
    >
        ...
        <Flex justify="space-between" align="center">
          ...
          <Flex gap={8}>
            ...
            <Button
              ...
              title={!selectedNode ? '请先选择一个组织' : '分配未归属用户'}
            >
              分配
            </Button>
            {/* 新增用户按钮 */}
      ...
      <Modal
        ...
        okText="确认分配"
        cancelText="取消"
        confirmLoading={batchAssignUsersMutation.isPending}
...
const UserCard: React.FC<UserCardProps> = ({ user, isSelected, onClick, deptMap }) => {
  ...
  return (
    ...
    >
      {/* 头像 */}
      <Badge dot color={getStatusColor(statusConfig.color)} offset={[-4, 36]}>
        <Avatar
          size={40}
          src={user.avatar}
          icon={<UserOutlined />}
