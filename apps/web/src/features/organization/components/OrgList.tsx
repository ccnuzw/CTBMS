...
export const OrgList: React.FC = () => {
  ...
  const orgTypeMap = useMemo(() => {
    ...
    const map = items.reduce<Partial<Record<OrganizationType, { label: string; color: string }>>>(
      (acc, item) => {
    ...
    return { ...ORG_TYPE_MAP_FALLBACK, ...map } as Record<
      OrganizationType,
  ...
  const requestOrganizations = async (
    ...
    if (codeKeyword) {
      data = data.filter((item) => item.code.toLowerCase().includes(codeKeyword));
    }

    const typeFilters = filter.type;
    if (typeFilters && typeFilters.length > 0) {
      const typeSet = new Set(typeFilters.map(String));
      data = data.filter((item) => typeSet.has(item.type));
    }
    ...
    if (sortField && sortOrder) {
      data = [...data].sort((a: any, b: any) => {
        ...
        if (left === undefined || left === null) return sortOrder === 'ascend' ? -1 : 1;
        if (right === undefined || right === null) return sortOrder === 'ascend' ? 1 : -1;
        return sortOrder === 'ascend' ? (left > right ? 1 : -1) : left > right ? -1 : 1;
      });
    }
