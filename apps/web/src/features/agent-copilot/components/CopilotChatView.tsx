...
const getQuickPhrases = (status: string, hasResult: boolean, hasNoSession: boolean): string[] => {
    if (hasNoSession || status === 'INTENT_CAPTURE') {
        return ['玉米价格走势', '持仓风险扫描', '周度复盘报告', '竞品对比分析'];
    }
    if (status === 'DONE' && hasResult) {
        return ['导出报告', '发到邮箱', '对比上周数据', '深入分析'];
    }
    if (status === 'FAILED') {
        return ['换个角度试试', '缩小分析范围', '重新分析'];
    }
...
const groupSessionsByDate = <T extends { updatedAt: string }>(sessions: T[]): Array<{ label: string; items: T[] }> => {
    const now = new Date();
...
const useTypingEffect = (text: string, isActive: boolean, speed = 25): { displayText: string; isTyping: boolean } => {
...
export const CopilotChatView: React.FC<CopilotChatViewProps> = ({ isAdminUser, onSwitchToAdmin }) => {
    ...
    const typingResult = useTypingEffect(
        ...
        Boolean(latestAssistantTurn && !localTurns.some((lt) => lt.id === latestAssistantTurn.id && lt.pending)),
    );
    ...
    const isAutoExecuted = allTurns.some(
        (t) => t.role === 'ASSISTANT' && (t.structuredPayload as Record<string, unknown>)?.autoExecuted === true,
    );
    const showConfirmCard = latestPlan && !latestPlan.isConfirmed && currentStatus === 'PLAN_PREVIEW' && !isAutoExecuted;
                ...
                <Row gutter={16}>
                    ...
                    <Col xs={24} md={18} lg={19}>
                        <Card bodyStyle={{ height: isMobile ? 'calc(100vh - 180px)' : 700, display: 'flex', flexDirection: 'column' }}>
                            {/* 消息流 */}
                            <div ref={timelineRef} onScroll={handleTimelineScroll} style={{ flex: 1, overflow: 'auto', marginBottom: 12, position: 'relative' }}>
                                {allTurns.length ? (
                                    <Space direction="vertical" style={{ width: '100%' }} size={12}>
                                        ...
                                        {visibleTurns.map((turn, turnIndex) => {
                                            ...
                                            const prevTurnTime = turnIndex > 0 ? visibleTurns[turnIndex - 1].createdAt : undefined;
                                            const showTimeSep = shouldShowTimeSeparator(prevTurnTime, turn.createdAt);
                                            const retryMsg = isFailedTurn ? ((turnPayload.retryMessage as string) || '') : '';
                                                ...
                                                <React.Fragment key={turn.id}>
                                                    ...
                                                    {showTimeSep ? (
                                                        <div style={{ textAlign: 'center', margin: '4px 0' }}>
                                                            <Text type="secondary" style={{ fontSize: 11, background: '#fff', padding: '0 8px' }}>
                                                                {formatTurnTime(turn.createdAt)}
                                                            </Text>
                                                        </div>
                                                    ) : null}
                                                    <div
                                                        className={roleText === 'USER' ? 'copilot-msg-right' : 'copilot-msg-left'}
                                                        style={{
                                                            display: 'flex',
                                                    ...
                                                    >
                                                        ...
                                                        <div style={{
                                                            ...
                                                            background: roleText === 'USER' ? '#e6f4ff' : 'linear-gradient(135deg, #667eea, #764ba2)',
                                                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                                                            fontSize: 14, flexShrink: 0, color: roleText === 'USER' ? '#1677ff' : '#fff',
                                                        }}>
                                                            {roleText === 'USER' ? '👤' : '🤖'}
                                                        </div>
                                                        <div style={{
                                                            ...
                                                            {isAssistant ? (
                                                                <div className={`copilot-markdown${typingResult.isTyping && turn.id === latestAssistantTurn?.id ? ' copilot-typing-cursor' : ''}`} style={{ lineHeight: 1.7 }}>
                            ...
                            {/* ── 确认卡片（对话式气泡风格） ── */}
                            {showConfirmCard ? (
                                <div className="copilot-card-in" style={{
                            ...
                            {quickPhrases.length > 0 ? (
                                <div style={{ marginBottom: 6, overflowX: 'auto', whiteSpace: 'nowrap', paddingBottom: 2 }}>
                                    <Space size={6}>
