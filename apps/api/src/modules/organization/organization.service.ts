...
export class OrganizationService {
  ...
  async remove(id: string): Promise<Organization> {
    ...
    // 检查是否有子组织
    const [childrenCount, departmentsCount] = await Promise.all([
      this.prisma.organization.count({ where: { parentId: id } }),
  ...
  private async findDescendantIds(id: string): Promise<string[]> {
    const allNodes = await this.prisma.organization.findMany({
