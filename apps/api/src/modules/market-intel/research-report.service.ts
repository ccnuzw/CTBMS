...
import {
  ...
  ReportPeriod as PrismaReportPeriod,
...
export class ResearchReportService {
  ...
  private mapInsightsToKeyPoints(insights?: AIAnalysisResult['insights']) {
    ...
    return insights.map((i) => ({
      ...
      sentiment:
        i.direction === 'Bullish' ? 'positive' : i.direction === 'Bearish' ? 'negative' : 'neutral',
  ...
  async update(id: string, dto: UpdateResearchReportDto) {
    ...
    try {
      ...
      console.warn('[ResearchReportService.update] Knowledge V2 sync failed:', error);
    }
  ...
  async export(ids?: string[], query?: ResearchReportQuery) {
    ...
    if (ids && ids.length > 0) {
      ...
      const { reportType, reviewStatus, commodity, region, startDate, endDate, keyword } =
        query || {};
      const where: Prisma.ResearchReportWhereInput = {};
      if (reportType) where.reportType = this.toPrismaReportType(reportType as TypesReportType);
      if (reviewStatus)
        where.reviewStatus = this.toPrismaReviewStatus(reviewStatus as TypesReviewStatus);
      if (commodity) where.commodities = { has: commodity };
      if (region) where.regions = { has: region };
      if (startDate || endDate) {
        where.publishDate = {
  ...
  async promoteToReport(intelId: string, dto: PromoteToReportDto) {
    ...
    const dataPoints =
      ...
      aiAnalysis.pricePoints?.slice(0, 5).map((p) => ({
  ...
  async getStats(options?: { days?: number }) {
    ...
    const [byType, byStatus, bySource, total, recent, totalViews, totalDownloads] =
      await Promise.all([
        // 按类型统计
