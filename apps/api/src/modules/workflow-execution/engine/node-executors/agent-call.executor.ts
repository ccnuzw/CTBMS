...
export class AgentCallNodeExecutor implements WorkflowNodeExecutor {
  ...
  async execute(context: NodeExecutionContext): Promise<NodeExecutionResult> {
    ...
    // 7. 拉取绑定的外挂技能 AgentSkill (Tool Definitions)
    const skillCodes = Array.isArray(profile.skillCodes) ? (profile.skillCodes as string[]) : [];
    let tools: AIToolDefinition[] | undefined = undefined;
    let availableSkills: AvailableSkill[] = [];
    if (skillCodes.length > 0) {
      availableSkills = await this.prisma.agentSkill.findMany({
        where: { skillCode: { in: skillCodes }, isActive: true },
      });
      tools = availableSkills.map((s) => ({
        type: 'function',
        function: {
          name: s.skillCode, // use skillCode as function name
          description: s.description ?? undefined,
          parameters: this.toUnknownRecord(s.parameters),
        },
      }));
    ...
    try {
      ...
      if (tools && provider.generateChat) {
        // Feature: Multi-turn Chat with Tool Calling
        ...
        while (currentTurn < MAX_TURNS) {
          currentTurn++;
          this.logger.log(`Agent[${agentCode}] 聊天回合开始 (Turn ${currentTurn})`);

          const chatResponse = await provider.generateChat(messages, requestOptions);

          if (chatResponse.tool_calls && chatResponse.tool_calls.length > 0) {
            // 模型要求调用工具
            this.logger.log(
              `Agent[${agentCode}] 命中工具调用: ${chatResponse.tool_calls.map((t) => t.function.name).join(', ')}`,
            );

            // 为了结构完整，先将 assistant 的回复(带有 tool_calls 声明)记录到消息历史中
            messages.push({
              role: 'assistant',
            ...
            for (const toolCall of chatResponse.tool_calls) {
              const skill = availableSkills.find((s) => s.skillCode === toolCall.function.name);
        ...
        if (!rawResponse) {
          rawResponse = '获取结果超时或未获得有效结论(超出了最大工具调用轮数)';
        }
      } else {
        // Legacy Fallback / Basic Completion
      ...
      // 重试策略 (简化版：由于多轮复杂性，这里对于失败进行全局退避重试，实际生产中可优化为只重试发生错误的特定回合)
      ...
      if (retryCount > 0) {
        ...
        if (retryResult.success) {
          ...
          return await this.buildInvocationFailureResult(context, agentCode, errorMsg, retryCount);
        }
      } else {
    ...
    const baseFieldResult = this.validateBaseOutputFields(
      parsedOutput,
  ...
  /**
   * 执行本地工具/外部接口 (Tool Calling Handler)
   */
  /**
  ...
  private async executeLocalTool(
    skill: { handlerCode: string | null } | undefined,
    argsStr: string,
    functionName: string,
    executionContext: NodeExecutionContext,
  ): Promise<string> {
    this.logger.log(
      `>> Preparing to execute logic behind skill <${functionName}> with args: ${argsStr}`,
    );

    let args: Record<string, unknown>;
    try {
      const parsed = typeof argsStr === 'string' ? JSON.parse(argsStr) : argsStr;
      args = this.toUnknownRecord(parsed) ?? {};
    } catch {
      this.logger.warn(`Failed to parse arguments for tool ${functionName}: ${argsStr}`);
      return JSON.stringify({ error: 'Invalid JSON arguments provided.' });
    }

    const handlerCode = skill?.handlerCode;

    if (!handlerCode) {
      this.logger.warn(`Skill ${functionName} does not have a mapped local handlerCode.`);
      return JSON.stringify({
        error: `Not Implemented. Local mapping for ${functionName} not found.`,
      });
    }

    // 动态路由并请求 ToolHandlerRegistry
    const handler = this.toolRegistry.getHandler(handlerCode);
    if (!handler) {
      this.logger.warn(`Could not find a registered ToolHandler for code [${handlerCode}]`);
      return JSON.stringify({
        error: `Not Implemented. The backend handler [${handlerCode}] is pending deployment.`,
      });
    }

    try {
      this.logger.log(`>> Routing to ToolHandler [${handlerCode}]...`);
      const reply = await handler.execute(args, executionContext);
      return reply;
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      this.logger.error(`Handler [${handlerCode}] execution failed`, error);
      return JSON.stringify({ error: `Business logic error during execution: ${message}` });
    }
  }

  private toUnknownRecord(value: unknown): Record<string, unknown> | undefined {
