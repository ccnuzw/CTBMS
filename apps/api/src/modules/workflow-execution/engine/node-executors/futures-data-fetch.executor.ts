...
export class FuturesDataFetchNodeExecutor implements WorkflowNodeExecutor {
    ...
    try {
        ...
        return {
          ...
          output: {
            ...
            metadata: {
              ...
              symbol,
              message:
                '当前使用模拟数据。接入真实交易所数据源后，请将 useMockData 设为 false 并配置 connectorCode。',
            },
          },
      ...
      if (connectorType !== 'EXCHANGE_API' && connectorType !== 'REST_API') {
        return {
          status: 'FAILED',
          output: {
            error: `期货数据节点仅支持 EXCHANGE_API 或 REST_API 类型连接器，当前: ${connectorType}`,
          },
          message: `连接器类型不兼容: ${connectorType}`,
        };
      }

      const contractValidation = validateConnectorContract(connector as Record<string, unknown>, {
      ...
      if (!contractValidation.valid) {
        return {
          status: 'FAILED',
          output: { error: `连接器契约缺失字段: ${contractValidation.missingFields.join(', ')}` },
          message: `连接器契约校验失败: ${connectorCode}`,
        };
      }

      // 构建交易所 API 请求
      const endpointConfig = (connector as Record<string, unknown>).endpointConfig as Record<
        string,
        unknown
      > | null;
      ...
      const rateLimitConfig = (connector as Record<string, unknown>).rateLimitConfig as Record<
        string,
        unknown
      > | null;
      ...
      try {
        ...
        // 应用响应映射
        const responseMapping = (connector as Record<string, unknown>).responseMapping as Record<
          string,
          string
        > | null;
  ...
  private generateMockKlineData(
    ...
    const totalBars = Math.min(Math.ceil((lookbackDays * 24 * 60 * 60 * 1000) / intervalMs), 500);
