  ...
  for (const item of items) {
    ...
    try {
      ...
      if (content.trim().length > 0) {
        const ingestResult = await ragService.ingest(item.id, content, {
        ...
        if (ingestResult.errorCode) {
          logger.warn(
            `Vectorized with issues ${item.id}: ${ingestResult.errorCode} (${ingestResult.errorMessage || 'no details'})`,
          );
        } else {
          logger.log(
            `Successfully vectorized ${item.id}: ${ingestResult.storedChunks}/${ingestResult.requestedChunks} chunks`,
          );
        }
      } else {
