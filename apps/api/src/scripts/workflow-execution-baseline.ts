...
type BaselineOptions = {
  ...
  triggerType?: 'MANUAL' | 'API' | 'SCHEDULE' | 'EVENT' | 'ON_DEMAND';
...
const parseOptions = (): BaselineOptions => {
  ...
  const triggerType = readArgValue(args, '--trigger-type')?.trim().toUpperCase();
  const mode = readArgValue(args, '--mode')?.trim().toUpperCase();
  const usageMethod = readArgValue(args, '--usage-method')?.trim().toUpperCase();
  ...
  return {
    ...
    triggerType: parseEnum(
      triggerType,
      ['MANUAL', 'API', 'SCHEDULE', 'EVENT', 'ON_DEMAND'] as const,
      '--trigger-type',
    ),
    mode: parseEnum(mode, ['LINEAR', 'DAG', 'DEBATE'] as const, '--mode'),
    usageMethod: parseEnum(
      usageMethod,
      ['HEADLESS', 'COPILOT', 'ON_DEMAND'] as const,
      '--usage-method',
    ),
...
const buildWhere = (options: BaselineOptions, since: Date): Prisma.WorkflowExecutionWhereInput => {
  ...
  if (options.ownerUserId || options.workflowDefinitionId || options.mode || options.usageMethod) {
    where.workflowVersion = {
      workflowDefinition: {
        ...
        ...(options.usageMethod ? { usageMethod: options.usageMethod } : {}),
      },
    };
  }
...
async function main() {
  ...
  const runId = `wf-execution-baseline-${startedAt
    .toISOString()
    .replace(/[-:.TZ]/g, '')
  ...
  const hasRateThresholds =
    resolvedThresholds.minSuccessRate !== undefined ||
  ...
  if (totalExecutions > 0) {
    if (
      resolvedThresholds.minSuccessRate !== undefined &&
      successRate < resolvedThresholds.minSuccessRate
    ) {
      violations.push(
        `successRate=${successRate} < minSuccessRate=${resolvedThresholds.minSuccessRate}`,
      );
    }
    if (
      resolvedThresholds.maxFailureRate !== undefined &&
      failedRate > resolvedThresholds.maxFailureRate
    ) {
      violations.push(
        `failedRate=${failedRate} > maxFailureRate=${resolvedThresholds.maxFailureRate}`,
      );
    }
    if (
      resolvedThresholds.maxCanceledRate !== undefined &&
      canceledRate > resolvedThresholds.maxCanceledRate
    ) {
      violations.push(
        `canceledRate=${canceledRate} > maxCanceledRate=${resolvedThresholds.maxCanceledRate}`,
      );
    }
    if (
      resolvedThresholds.maxTimeoutRate !== undefined &&
      timeoutRate > resolvedThresholds.maxTimeoutRate
    ) {
      violations.push(
        `timeoutRate=${timeoutRate} > maxTimeoutRate=${resolvedThresholds.maxTimeoutRate}`,
      );
    }
  ...
  if (durationValues.length > 0) {
    if (
      resolvedThresholds.maxP95DurationMs !== undefined &&
      p95 > resolvedThresholds.maxP95DurationMs
    ) {
      violations.push(
        `p95DurationMs=${p95} > maxP95DurationMs=${resolvedThresholds.maxP95DurationMs}`,
      );
    }
  ...
  const report: BaselineReport = {
    ...
    query: {
      ...
      mode: options.mode,
      usageMethod: options.usageMethod,
    },
    totals: {
  ...
  .catch((error) => {
    if (error instanceof Error) {
      process.stderr.write(
        `[workflow-execution-baseline] fatal: ${error.message}\n${error.stack ?? ''}\n`,
      );
    } else {
