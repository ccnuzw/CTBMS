...
async function main() {
  ...
  try {
    await prisma.user.create({
      data: {
        id: ownerUserId,
        username: `${token}_owner`,
        email: `${token}_owner@example.com`,
        name: `Owner ${token}`,
      },
    });
    ...
    const listVersions = await fetchJson<
      Array<{
        id: string;
        versionCode: string;
        status: string;
    ...
    assert.ok(
      listVersions.body.some((item) => item.id === initialVersionId && item.status === 'DRAFT'),
    );
    assert.ok(
      listVersions.body.some(
        (item) => item.id === createVersion.body.id && item.status === 'PUBLISHED',
      ),
    );
    assert.ok(
      listVersions.body.some((item) => item.versionCode === '1.0.2' && item.status === 'DRAFT'),
    );
