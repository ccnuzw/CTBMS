...
const createDslSnapshot = (params: {
  ...
  nodes: [
    ...
    },
    {
      ...
      enabled: true,
      config: {
        delaySeconds: (params.passthroughDelayMs ?? 0) / 1000,
        delayMs: params.passthroughDelayMs ?? 0,
      },
      runtimePolicy: {
        timeoutSeconds: Math.max(1, Math.ceil((params.passthroughTimeoutMs ?? 30_000) / 1000)),
...
async function main() {
  ...
  try {
    ...
    const triggerFirst = await fetchJson<{ id: string; status: string }>(
    ...
    const triggerSecond = await fetchJson<{ id: string; status: string }>(
      `${baseUrl}/workflow-executions/trigger`,
      {
        method: 'POST',
        headers: {
    ...
    const triggerCancelablePromise = fetchJson<{ id: string; status: string }>(
      `${baseUrl}/workflow-executions/trigger`,
      {
        method: 'POST',
        headers: {
    ...
    const publicTriggerPromise = fetchJson<{ id: string; status: string }>(
      `${baseUrl}/workflow-executions/trigger`,
      {
        method: 'POST',
        headers: {
    ...
    const ownerVisibilityTrigger = await fetchJson<{ id: string; status: string }>(
      `${baseUrl}/workflow-executions/trigger`,
      {
        method: 'POST',
        headers: {
    ...
    const outsiderVisibilityTrigger = await fetchJson<{ id: string; status: string }>(
      `${baseUrl}/workflow-executions/trigger`,
      {
        method: 'POST',
        headers: {
    ...
    assert.ok(
      outsiderVisibilityList.body.data.some(
        (item) => item.id === outsiderVisibilityTrigger.body.id,
      ),
    );
    assert.ok(
      !outsiderVisibilityList.body.data.some((item) => item.id === ownerVisibilityTrigger.body.id),
    );
    ...
    assert.ok(
      ownerVisibilityList.body.data.some((item) => item.id === ownerVisibilityTrigger.body.id),
    );
    assert.ok(
      ownerVisibilityList.body.data.some((item) => item.id === outsiderVisibilityTrigger.body.id),
    );
    ...
    const timeoutTrigger = await fetchJson<{ message?: string }>(
      `${baseUrl}/workflow-executions/trigger`,
      {
        method: 'POST',
        headers: {
    ...
    const failedNodeExecution = timeoutExecution?.nodeExecutions.find(
    ...
    const publicTimeoutTrigger = await fetchJson<{ message?: string }>(
      `${baseUrl}/workflow-executions/trigger`,
      {
        method: 'POST',
        headers: {
...
main().catch((error) => {
  if (error instanceof Error) {
    process.stderr.write(
      `[workflow-p2-reliability.e2e] failed: ${error.message}\n${error.stack ?? ''}\n`,
    );
  } else {
