...
class AgentConversationConflictsE2eModule implements NestModule {
...
const buildDsl = (workflowId: string, workflowName: string, rulePackCode: string) => ({
  ...
  nodes: [
    ...
    { id: 'n_rule_eval', type: 'rule-eval', name: 'rule eval', enabled: true, config: { rulePackCode } },
    {
      ...
      name: 'risk gate',
      enabled: true,
      config: { riskProfileCode: 'AGENT_CONVERSATION_CONFLICT_E2E' },
    },
...
async function main() {
  const app = await NestFactory.create(AgentConversationConflictsE2eModule, {
    logger: ['error', 'warn'],
  });
  ...
  const token = `agent_conflicts_${Date.now()}`;
  const workflowId = `${token}_workflow`;
  const workflowName = `Agent Conflicts ${token}`;
  ...
  try {
    ...
    const createDefinition = await fetchJson<{
      ...
      body: JSON.stringify({
        ...
        changelog: 'agent conflicts create',
      }),
    });
    ...
    const createSession = await fetchJson<{ id: string }>(`${baseUrl}/agent-conversations/sessions`, {
      ...
      body: JSON.stringify({ title: 'Conflicts Session' }),
    });
    ...
    const conflicts = await fetchJson<{
    ...
    }>(`${baseUrl}/agent-conversations/sessions/${conversationSessionId}/conflicts`, {
      headers: {
        'x-virtual-user-id': ownerUserId,
      },
    });
    assert.equal(conflicts.status, 200);
    assert.equal(typeof conflicts.body.consistencyScore, 'number');
    assert.ok(Array.isArray(conflicts.body.conflicts));
    if (conflicts.body.conflicts.length > 0) {
      const first = conflicts.body.conflicts[0];
      assert.ok(first.conflictId);
...
main()
  .then(() => {
    process.stdout.write('agent-conversation-conflicts e2e passed\n');
  })
  .catch((error) => {
    process.stderr.write(
      `agent-conversation-conflicts e2e failed: ${error instanceof Error ? error.stack : String(error)}\n`,
    );
    process.exit(1);
  });
