...
export const BatchAssignUsersSchema = z
  .object({
    userIds: z.array(z.string().uuid()).min(1, '请至少选择一个用户'),
    organizationId: z.string().uuid().nullable().optional(),
    departmentId: z.string().uuid().nullable().optional(),
  })
  .refine((value) => Boolean(value.organizationId || value.departmentId), {
    message: 'organizationId 或 departmentId 至少提供一个',
    path: ['organizationId'],
  });

const UserIdListSchema = z.preprocess((value) => {
...
export type BatchAssignUsersDto = z.infer<typeof BatchAssignUsersSchema>;
