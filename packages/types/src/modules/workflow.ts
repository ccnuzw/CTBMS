...
export const WorkflowDslSchema = z.object({
  ...
  dataConnectorBindings: z.array(z.string()).optional(),
...
const QueryBooleanWithDefaultTrueSchema = QueryBooleanOptionalSchema.transform(
  (value) => value ?? true,
);
...
export const WorkflowExecutionQuerySchema = z
  .object({
  ...
  .refine(
    (value) =>
      !value.startedAtFrom || !value.startedAtTo || value.startedAtFrom <= value.startedAtTo,
    {
...
export const WorkflowPublishAuditQuerySchema = z
  .object({
    workflowVersionId: z.string().uuid().optional(),
  ...
  .refine(
    (value) =>
      ...
      !value.publishedAtTo ||
      value.publishedAtFrom <= value.publishedAtTo,
    {
