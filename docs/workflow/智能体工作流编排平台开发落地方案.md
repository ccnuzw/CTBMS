...
| P2-04  | 执行策略可配置化（超时/重试/退避） | `apps/api/src/modules/workflow-definition/**`、`apps/api/src/modules/workflow-execution/**`、`apps/web/src/features/workflow-studio/**` | 已支持 `dsl.runPolicy/node.runtimePolicy` 透传；发布前新增运行策略覆盖校验，创建页支持默认策略配置入口               | 已完成 |

当前进展（P2-04）：

1. 发布前 DSL 校验已新增 `WF106`：非触发节点必须在 `node.runtimePolicy/node.config/runPolicy.nodeDefaults` 中具备完整运行策略字段（`timeoutMs/retryCount/retryBackoffMs/onError`）。
2. 后端默认 DSL（未传 `dslSnapshot` 场景）已补齐 `runPolicy.nodeDefaults`，避免默认创建流程在发布阶段因策略缺失被阻断。
3. 前端定义创建页已支持默认运行策略配置，并回写至 `dslSnapshot.runPolicy.nodeDefaults`。
4. 回归验证已通过：`workflow-p1-core.e2e`、`workflow-p2-reliability.e2e`、`pnpm type-check`、`pnpm workflow:summary:self-check:all`。

### 19.3 批次 P3（前端编排与运行监控 MVP）

| 任务ID | 小任务                     | 输出物                                      | 验收标准                                   | 状态   |
| ------ | -------------------------- | ------------------------------------------- | ------------------------------------------ | ------ |
| P3-01  | 工作流定义列表与版本管理页 | `apps/web/src/features/workflow-*`          | 可查看定义、版本、发布记录                 | 已完成 |
| P3-02  | 运行实例列表与时间线回放页 | `apps/web/src/features/workflow-runtime/**` | 可查看运行详情、节点执行、runtime timeline | 已完成 |
| P3-03  | 发布审计与运行筛选联动     | `apps/web/src/features/workflow-*`          | 前端筛选项与后端 query 1:1 对齐            | 已完成 |

当前进展（P3）：

1. 定义管理页已支持定义列表、版本管理、发布操作与发布审计分页查看。
2. 运行实例页已支持失败分类/失败代码筛选、取消执行、节点执行详情与 runtime timeline 回放。
3. runtime timeline 已支持事件类型/级别筛选和分页，和后端 timeline query 字段保持对齐。
4. 发布审计已支持版本/发布人/发布时间区间筛选，并和后端 query 字段对齐。

P3-03 对齐清单（已落地）：

1. 运行实例筛选：``。
2. 运行时间线筛选：`eventType/level/page/pageSize`。
3. 定义列表筛选：`keyword/mode/usageMethod/status/includePublic/page/pageSize`。
4. 发布审计筛选：`workflowVersionId/publishedByUserId/publishedAtFrom/publishedAtTo/page/pageSize`。

### 19.4 批次 P4（上线收口）

| 任务ID | 小任务                  | 输出物                                              | 验收标准                                 | 状态   |
| ------ | ----------------------- | --------------------------------------------------- | ---------------------------------------- | ------ |
| P4-01  | 权限与审计收口          | `apps/api/src/modules/*`、`apps/web/src/features/*` | 发布/执行/回放权限边界清晰，审计字段完整 | 已完成 |
| P4-02  | 观测指标与告警基线      | `scripts/*`、`docs/*`                               | 成功率/失败率/耗时可监控，异常可告警     | 已完成 |
| P4-03  | 部署与回滚 Runbook 收口 | `DEPLOYMENT.md`、`docs/*`                           | 支持灰度发布与快速回滚演练               | 已完成 |

当前进展（P4-01）：

1. 执行实例变更与读取权限已收紧：`cancel/rerun/findOne/timeline/findAll` 均按“执行触发人或流程 owner”可见性判定，避免公共流程被旁路用户查看或改写状态。
2. 发布审计查询已支持 `workflowVersionId/publishedByUserId/publishedAtFrom/publishedAtTo`，前端定义页已完成对应筛选联动。
3. 回归补齐：`workflow-p1-core.e2e-spec.ts` 新增发布审计筛选断言；`workflow-p2-reliability.e2e-spec.ts` 新增“公共流程 outsider 取消失败 + outsider 不可查看 owner 执行详情/时间线 + outsider 不可重跑 owner 失败实例”断言。

当前进展（P4-02）：

1. 新增 `workflow:execution:baseline` 脚本，产出执行质量基线报告（成功率/失败率/取消率/超时率 + p50/p90/p95/p99）。
2. 新增 `workflow:execution:baseline:gate` 阈值门禁命令，支持按阈值自动失败并输出违规项。
3. 报告默认输出：`apps/api/logs/workflow-execution-baseline-report.json`（可用 `--report-file` 覆盖）。
4. `Workflow Smoke` CI 已接入 baseline gate、Step Summary 摘要和 artifact 归档，纳入最终门禁。
5. `workflow-ci-step-summary` 已并入 baseline 渲染区块，替换 workflow 内联 `node -e` 摘要逻辑；相关 `self-check` 已通过。
6. baseline gate 在“无样本窗口”场景下输出 `warnings` 并跳过阈值误报，避免空样本阻断 CI。
7. 新增 `workflow:execution:baseline:report:validate` 与 `workflow:execution:baseline:report:validate:self-check`，用于校验 baseline 报告契约并输出 validation json。
8. `Workflow Smoke` CI 已新增 baseline report validation gate，并将 `Workflow Execution Baseline Validation` 区块纳入 Step Summary 与 artifact 归档。
9. `workflow:drill:staging:precheck` 已串联 baseline report validate，staging 演练模板同步增加校验命令与验收项。
10. 新增 `workflow:execution:baseline:trend` 与 `workflow:execution:baseline:trend:self-check`，支持当前基线与参考基线对比（成功率/失败率/超时率/p95）并输出趋势报告。
11. `Workflow Smoke` CI 已接入 baseline trend compare，并将 `Workflow Execution Baseline Trend` 区块纳入 Step Summary 与 artifact 归档。
12. 新增 `workflow:execution:baseline:reference` 与 `workflow:execution:baseline:reference:self-check`，支持 reference 基线 `ensure/promote` 双模式维护，并输出 operation 报告。
13. `Workflow Smoke` CI 已接入 baseline reference 缓存 restore/save + ensure/promote 编排，trend compare 改为 `--require-reference` 强校验，避免 reference 缺失导致趋势判定弱化。
14. `workflow-ci-step-summary` 已新增 `Workflow Execution Baseline Reference Operation` 区块，便于在 CI 摘要中直接定位 reference 文件维护动作与异常原因。
15. trend 阈值已外置到 `config/workflow-execution-baseline-thresholds.json`，并支持 CLI 参数覆盖，降低 CI/脚本硬编码维护成本。
16. `workflow:drill:staging:precheck` 已扩展为完整 reference 生命周期（ensure -> trend -> promote）串联校验。
17. 新增 `workflow:execution:baseline:reference:ci-state` 与对应 `self-check`，`Workflow Smoke` CI/Step Summary 已接入 `Workflow Execution Baseline Reference CI State` 区块（含 cache 命中、ensure/trend/promote 结果）。

当前进展（P4-03）：

1. `DEPLOYMENT.md` 已新增“工作流上线门禁、灰度观察窗口、回滚 Runbook（含数据库前向修复策略）”。
2. 已新增 `docs/工作流生产值班与回滚SOP.md`，覆盖发布前检查、灰度观察、故障分级、回滚后验收模板和值班交接模板。
3. 已新增 `docs/工作流Staging演练记录模板.md`，覆盖预检查命令、链路演练、权限演练、回滚演练与问题闭环模板。
4. 新增 `workflow:drill:staging:precheck` 组合命令，便于演练前快速门禁校验。
5. 已完成一次 staging precheck 实战预演并归档：`docs/工作流Staging演练记录-2026-02-12.md`（含报告产物路径与阻塞项）。
6. 已新增 `workflow:drill:staging:precheck:fast` 快通道（分包 type-check，规避 turbo/keychain 环境阻断），并同步模板/SOP。
7. 已修复 baseline gate 报告路径透传偏差（root/api 路径统一），避免 precheck 在 report validate 阶段误失败。
8. 已新增 `workflow:drill:rollback:verify` 回滚后验收组合命令，降低回滚演练执行成本并统一产物路径。
9. 已完成 `workflow:drill:rollback:verify` 回滚实战（本地等价环境），核心产物已生成：
   - `logs/workflow-smoke-gate-report.json`
   - `logs/workflow-execution-baseline-post-rollback.json`
   - `logs/workflow-execution-baseline-post-rollback-validation.json`
   - `logs/workflow-execution-baseline-post-rollback-trend.json`
10. 已完成 Step Summary 本地联动确认：`pnpm workflow:ci:step-summary -- --execution-baseline-report-file=... --execution-baseline-validation-file=... --execution-baseline-trend-file=...` 可渲染 baseline 区块并读取本轮产物。
11. 已完成 precheck 稳定性增强：`has-risk-summary/execution-filters` 冒烟脚本固定监听 `127.0.0.1`、增加 fetch 超时与 DB 重试日志；`workflow-smoke-runner` 对关键 DB 依赖步骤增加瞬时故障重试。
12. 已新增一键全流程演练命令：`pnpm workflow:drill:staging:full`（`precheck:fast + rollback:verify`），用于缩短人工串行执行成本。
13. 已新增 `workflow:ci:step-summary:validate`（含 `self-check`）并接入 `Workflow Smoke` CI，自动校验 Step Summary 必含 baseline 关键区块，避免人工漏检。
14. 已完成 `workflow:drill:staging:full` 本地等价实跑，通过“预检 + 回滚验收”整链路复验。
15. 已新增 `workflow:drill:staging:full:summary`（含 `self-check`），将 precheck/rollback/CI-summary 校验结果汇总为单一总报告（`logs/workflow-drill-staging-full-summary.{md,json}`）。
16. 已新增 `workflow:drill:staging:closeout`（含 `self-check`），将 `staging-full-summary + ci-step-summary-validation + CI run evidence` 收口为单一上线结论报告（`logs/workflow-drill-staging-closeout.{md,json}`），并支持 `--require-ci-run-url --require-ci-run-success` 严格门禁。
17. `workflow-ci-step-summary` 已新增 `Workflow Staging Drill Closeout` 区块，并由 `workflow:ci:step-summary:validate` 强制校验必含该区块，避免发布收口信息在摘要中缺失。
18. `workflow:drill:staging:full` 已升级为“双阶段摘要”编排：closeout 前先以 `--ignore-staging-drill-closeout` 生成并校验基础摘要，closeout 后再次生成并校验最终摘要，避免读取历史 closeout 残留导致误判。
19. 已完成“Workflow Smoke CI 实跑链接回填至演练记录”并复跑严格 closeout：`pnpm workflow:drill:staging:closeout -- --ci-run-url=... --ci-run-conclusion=SUCCESS --require-ci-run-url --require-ci-run-success`，当前结论 `READY_TO_PROMOTE`（证据见 `docs/工作流Staging演练记录-2026-02-12.md` 与 `logs/workflow-drill-staging-closeout.json`）。
20. 已完成阶段出口快照（2026-02-13）：

- `pnpm lint` 受本机 `turbo/keychain TLS` 环境限制无法执行（非代码问题）。
- 等价门禁命令 `pnpm -r --if-present run lint`：通过。
- `pnpm type-check`：通过。
- `pnpm --filter api run test:e2e`：通过（含 `workflow-p1-core/workflow-p2-reliability/trigger-gateway/execution-analytics/template-catalog` 全链路）。

21. 已新增执行基线脚本维度过滤能力：支持 `--trigger-type`、`--mode`、`--usage-method`，可按 `ON_DEMAND`/`DAG` 维度独立产出与门禁。
22. 已新增 API e2e 回归：`apps/api/test/workflow-dag.e2e-spec.ts`、`apps/api/test/workflow-debate-execution.e2e-spec.ts`，覆盖 DAG 并行汇聚和 Debate 执行链路。
23. 已新增对齐文档：`docs/工作流接口命名映射与实施对齐清单.md`、`docs/工作流发布DefinitionOfLaunch验收模板.md`，用于设计稿与实现收口。
24. 已新增接口别名入口：`/workflow-validations`、`/workflow-replays/:id*`（保持原有 `/workflow-definitions/validate-dsl` 与 `/workflow-executions/:id/*` 并存）。
25. 已新增别名路由回归：`apps/api/test/workflow-api-alias.e2e-spec.ts`（覆盖 validate/replay/timeline）。
26. `workflow:smoke:gate` 已扩展纳入 `workflow-dag`、`workflow-debate-execution`、`workflow-api-alias` e2e，并完成本地门禁通过。
27. `workflow:drill:staging:closeout` 严格门禁输出 `READY_TO_PROMOTE`，closeout 证据已固化到 `logs/workflow-drill-staging-closeout.json`（与 `docs/工作流Staging演练记录-2026-02-12.md` 对齐）。
