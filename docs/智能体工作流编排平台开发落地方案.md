...
| 任务ID | 小任务 | 输出物 | 验收标准 |
|---|---|---|---|
| C-01 | 执行引擎关键路径性能基线 | `apps/api/test/perf/*` 或性能报告文档 | 提供基线数据（P50/P95）并记录环境配置 |
| C-02 | 风险闸门节点契约文档固化 | `docs/工作流节点契约-riskgate.md` | 输入/输出字段、默认值、异常路径完整 |
| C-03 | 发布前回归命令聚合 | 根 `package.json`（`workflow:smoke:gate`） | `pnpm workflow:smoke` + 关键业务回归可一键执行 |
