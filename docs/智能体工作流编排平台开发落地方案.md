...
| P4-03  | 部署与回滚 Runbook 收口 | `DEPLOYMENT.md`、`docs/*`                           | 支持灰度发布与快速回滚演练               | 进行中 |
...
20. 已新增执行基线脚本维度过滤能力：支持 `--trigger-type`、`--mode`、`--usage-method`，可按 `ON_DEMAND`/`DAG` 维度独立产出与门禁。
21. 已新增 API e2e 回归：`apps/api/test/workflow-dag.e2e-spec.ts`、`apps/api/test/workflow-debate-execution.e2e-spec.ts`，覆盖 DAG 并行汇聚和 Debate 执行链路。
22. 已新增对齐文档：`docs/工作流接口命名映射与实施对齐清单.md`、`docs/工作流发布DefinitionOfLaunch验收模板.md`，用于设计稿与实现收口。
23. 已新增接口别名入口：`/workflow-validations`、`/workflow-replays/:id*`（保持原有 `/workflow-definitions/validate-dsl` 与 `/workflow-executions/:id/*` 并存）。
24. 已新增别名路由回归：`apps/api/test/workflow-api-alias.e2e-spec.ts`（覆盖 validate/replay/timeline）。
25. `workflow:smoke:gate` 已扩展纳入 `workflow-dag`、`workflow-debate-execution`、`workflow-api-alias` e2e，并完成本地门禁通过。
