...
| `INTEL_TASK_TYPE` | 任务类型（全局） | 简化后仅保留基础通用类型 | 任务管理/其他领域 |
| `INTEL_TASK_PRIORITY` | 任务优先级 | 低/中/高/紧急 | 任务列表 |
| `TIME_RANGE` | 时间范围 | 今日/本周/本月等 | 筛选器 |
| `RELATION_TYPE` | 关联类型 | 提及/主体/来源 | 情报关联 |

#### INTEL_TASK_TYPE 建议码表（简化）

| code | label | 说明 |
|---|---|---|
| `COLLECTION` | 采集任务 | 绑定采集点与结构化表单 |
| `REPORT` | 报告任务 | 日报/周报/月报统一类型 |
| `RESEARCH` | 调研任务 | 专题研究/深度调研 |
| `VERIFICATION` | 核实任务 | 临时核实/应急核对 |
| `OTHER` | 其他 | 兜底类型，可扩展 |

> 说明：任务类型为全局字典，必须绑定表单与审核流程；可在不同领域复用，按需扩展。
