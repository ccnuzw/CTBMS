...
## 📊 枚举与字典决策规范 (Enum vs Dictionary)

新增枚举时，必须根据以下决策树确定其归属：

```
新增枚举的判断流程：

┌─ 需要后台动态配置吗？（如新增选项无需发版）
│  ├─ 是 → 做成数据字典 (seed-dictionaries.ts)
│  └─ 否 ─┐
│         │
├─────────┘
│
├─ 与代码逻辑强耦合吗？（如状态机、条件分支判断）
│  ├─ 是 → 前端常量 (statusEnums.ts)
│  └─ 否 ─┐
│         │
├─────────┘
│
├─ 仅技术内部使用吗？（不面向业务用户）
│  ├─ 是 → 前端常量 (technicalEnums.ts)
│  └─ 否 ─┐
│         │
├─────────┘
│
├─ 使用范围狭窄吗？（仅特定功能模块）
│  ├─ 是 → 前端常量 (featureEnums.ts)
│  └─ 否 → 做成数据字典
```
...
| 归属 | 文件位置 | 适用场景 |
|---|---|---|
| **数据字典** | `apps/api/prisma/seed-dictionaries.ts` | 业务用户需要动态配置的选项 |
| **状态机常量** | `apps/web/src/constants/statusEnums.ts` | 状态流转、审核流程等 |
| **技术常量** | `apps/web/src/constants/technicalEnums.ts` | 开发者内部使用 |
| **功能常量** | `apps/web/src/constants/featureEnums.ts` | 特定功能模块专用 |
| **Types 包** | `packages/types/src/modules/*.ts` | Enum 类型定义 + 字典域保留的 LABELS |

### 单一事实来源

> [!IMPORTANT]
> **禁止重复定义**：同一 LABELS 只能存在于一处。
> - 字典域的 LABELS：保留在 `@packages/types`（12 个）
> - 非字典域的 LABELS：只在 `@/constants`（21 个）

详细规范见 `docs/data-dictionary.md`。
